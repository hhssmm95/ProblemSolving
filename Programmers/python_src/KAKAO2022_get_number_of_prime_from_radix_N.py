...
def getRadixKnumber(n,k):
    ...
    while (n >= k):
        remain = n % k
        result.insert(0, chr(remain + 48))
        n //= k
    
    if n > 0:
        result.insert(0, chr(n + 48))
...
def getDecimalCount(radixNum):
    #numArray = list(map(int,radixNum.split('0')))
    numArray = []

    number = ''
    for i in range(len(radixNum)):
        if radixNum[i] != '0':
            number+=radixNum[i]
...
def solution(n, k):
    radixNum = getRadixKnumber(n,k)
    return getDecimalCount(radixNum)
